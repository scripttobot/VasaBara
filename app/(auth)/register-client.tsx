...
export default function RegisterClientScreen() {
  ...
  const handleGoogleRegister = async () => {
    ...
    setLoading(true);
    const success = await googleLogin('client');
    setLoading(false);
    if (success) {
      router.replace('/(client)');
    } else {
  ...
  return (
    ...
    >
      <View style={styles.header}>
        <Text style={styles.title}>ভাড়াটিয়া রেজিস্ট্রেশন</Text>
        <Text style={styles.subtitle}>আপনার তথ্য দিয়ে অ্যাকাউন্ট তৈরি করুন</Text>
      </View>
      ...
      <View style={styles.form}>
        ...
        <View style={styles.inputGroup}>
          ...
          <View style={styles.chipRow}>
            {genderOptions.map(opt => (
              <Pressable
                key={opt.id}
                style={[styles.chip, gender === opt.id && styles.chipActive]}
        ...
        <View style={styles.inputGroup}>
          ...
          <View style={styles.inputWrapper}>
            ...
            <TextInput style={styles.input} placeholder="আপনার পেশা" placeholderTextColor={Colors.textMuted} value={occupation} onChangeText={setOccupation} />
          </View>
        </View>
        ...
        <View style={styles.inputGroup}>
          ...
          <View style={styles.chipRow}>
            {DIVISIONS.slice(0, 4).map(div => (
              <Pressable
                key={div.id}
                style={[styles.chip, selectedDivision === div.id && styles.chipActive]}
                onPress={() => setSelectedDivision(div.id)}
              >
                <Text style={[styles.chipText, selectedDivision === div.id && styles.chipTextActive]}>{div.nameBn}</Text>
              </Pressable>
            ))}
          </View>
          <View style={styles.chipRow}>
